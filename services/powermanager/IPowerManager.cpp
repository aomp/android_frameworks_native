...
enum {
    ...
    NOTIFY_FORCE_DISABLE_AAL = IBinder::FIRST_CALL_TRANSACTION + 25,
};
...
public:
	...
	virtual status_t notifyForceDisableAAL(const int32_t flag)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IPowerManager::getInterfaceDescriptor());
        data.writeInt32(flag);
        return remote()->transact(NOTIFY_FORCE_DISABLE_AAL, data, &reply);
    }
};
